...
(function(it){
  ...
  function compileScripts(){
    ...
    function walk(source, base, top){
      var dir, filename, jsPath, relativePath;
      dir = path.dirname(source);
      if (o.output) {
        filename = outputFilename(source, false);
        dir = path.join(o.output, dir.slice(base === '.'
          ? 0
          : base.length));
        source = path.normalize(source);
        jsPath = path.join(dir, filename);
        relativePath = path.relative(path.dirname(jsPath), source);
      } else {
        relativePath = null;
      }
      function work(){
        fshoot('readFile', source, function(it){
          compileScript(source, it + "", base, relativePath);
        });
      }
  ...
  function compileScript(filename, input, base, relativePath){
