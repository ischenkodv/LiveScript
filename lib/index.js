...
exports.compile = function(code, options){
  var result, ast, output, filename, outputFilename, relativePath, ref$, mapPath, e, that;
  ...
  try {
    if (options.json) {
      ...
      if (options.map && options.map !== 'none') {
        filename = options.filename, outputFilename = options.outputFilename, relativePath = options.relativePath;
        if (!filename) {
          filename = "unnamed-" + Math.floor(Math.random() * 4294967296).toString(16) + ".ls";
        }
        if (relativePath) {
          output.setFile(relativePath);
        } else {
          output.setFile(filename);
        }
        result = output.toStringWithSourceMap();
        if (options.map === 'embedded') {
          result.map.setSourceContent(relativePath != null ? relativePath : filename, code);
        }
